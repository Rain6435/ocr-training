...
\begin{abstract}
This report presents a technical evaluation of an intelligent, cost-efficient
OCR pipeline for historical document digitization. The system implements a
multi-stage architecture combining image preprocessing, document-difficulty
classification, confidence-aware routing, and multi-engine OCR inference. The
main engineering contribution is the integration of a custom CNN-LSTM-CTC
recognizer with Tesseract and TrOCR to operationalize cost-accuracy
trade-offs across document difficulty levels. The report
documents system design decisions, implementation details, empirical results
across disaggregated difficulty subsets, and lessons learned from multi-stage
...
hard) using Google Cloud Vertex AI with 30 epochs and batch size of 64 on 
an n1-standard-4 machine, which was powerful enough for this specific training. The resulting model achieved 65.67\% training 
accuracy, 65.28\% validation accuracy, and 62.78\% test accuracy with 
a final loss of 0.512 (training completed in 12 hours 18 minutes).
...
the reference. Word Error Rate (WER) is defined analogously at the
word level:
\[
  \text{WER} =
  \frac{\text{edit\_distance}_{w}(\mathit{pred},\mathit{truth})}{|\mathit{truth}_{w}|}
\]
Latency $t$ is measured in milliseconds (wall-clock),
...
A principled online variant can be written as a constrained problem:
\[
  \min_{\pi}\; \mathbb{E}[\text{cost}] \quad
  \text{s.t.} \quad \mathbb{E}[\text{risk}] \le \tau,
\]
where $\tau$ is a target risk budget. The corresponding Lagrangian is
\[
  \mathcal{L}(\pi, \lambda) =
  \mathbb{E}[\text{cost}] +
  \lambda\big(\mathbb{E}[\text{risk}] - \tau\big),
  \quad \lambda \ge 0.
\]
With online samples $(d_t, e_t)$, a projected dual-ascent update is
\[
  \lambda_{t+1} =
  \max\!\left(0,\, \lambda_t + \eta\,[\text{risk}(e_t \mid d_t) - \tau]\right),
\]
where $\eta > 0$ is the learning rate. This increases $\lambda$ when
observed risk exceeds the target and decreases it otherwise.
...
The classifier was trained on 14,403 labeled difficulty images (5,000
easy, 5,000 medium, 4,403 hard) using Google Cloud Vertex AI,
achieving 65.67\% training accuracy, 65.28\% validation accuracy, and
62.78\% test accuracy. As documented in Section~\ref{sec:classifier_eval},
there remains room for improvement in class-wise performance, particularly
on harder samples, suggesting that alternative architectures or expanded
training corpora could yield meaningful gains.
...
\paragraph{4. Custom OCR Model Architecture (CustomRNN / CRNN-CTC).}


The custom recognizer (CRNN-CTC in code; CustomRNN in this report)
follows the classical CNN-to-RNN-to-CTC design~\cite{ref:shi2015,ref:graves2006}. The
model consumes grayscale line images resized to
$64 \times 256 \times 1$, predicts a per-timestep distribution over
the vocabulary plus CTC blank, and decodes to variable-length text.

The visual backbone uses six convolutional layers grouped into
progressive blocks (64/128/256/256/512/512 channels), each with batch
normalization and ReLU activations. Pooling is deliberately
asymmetric in deeper layers ($(2,1)$) so that vertical resolution is
reduced while horizontal resolution (the text reading axis) is
preserved. A final $(4,1)$ convolution collapses the height dimension
to one row, yielding a sequence of feature vectors aligned with the
text direction.

Sequence modeling is performed by two bidirectional LSTM layers
($256$ units each direction, dropout $0.25$), followed by a dense
softmax output head. The training graph wraps the inference network
with a dedicated CTC-loss layer. This allows optimization without
character-level alignment while still exporting a clean inference-only
artifact.

The CustomRNN data path is also implementation-specific rather than
generic. The dataset loader builds fixed-shape batches from CSV
manifests, preserves aspect ratio during resize, pads to target width,
and computes \texttt{input\_length} deterministically from the CNN
downsampling factor ($T = W/4$). The vocabulary is explicitly defined
for uppercase/lowercase letters, digits, and common punctuation with a
stable index mapping to ensure reproducible encoding/decoding.

Training uses Adam ($\eta=10^{-3}$) with gradient clipping
(clip-norm $=1.0$), early stopping, learning-rate reduction on
plateau, and model checkpointing. In addition to \texttt{val\_loss},
the pipeline computes validation CER/WER each epoch through the
inference model, optionally using greedy or beam-search decoding and
optional dictionary-based post-correction.

Inference includes
robustness-oriented logic beyond a single forward pass: polarity
normalization (black text on white background), foreground cropping,
confidence-aware second-pass variants (enhanced/denoised/sharpened),
beam decoding (default width $10$), and a text-quality confidence
penalty for gibberish-like outputs. This design helps the CustomRNN
remain competitive on medium-difficulty historical lines while
retaining low per-sample cost.
...
Classifier and CustomRNN training is orchestrated via Google Cloud Vertex AI. A
...
$\text{CER}=\text{edit\_distance}(\mathit{pred},\mathit{truth})/|\mathit{truth}|$
in Section~2.1, because character-level Levenshtein distance with unit
operation costs satisfies
$\text{edit\_distance}=\text{substitutions}+\text{deletions}+\text{insertions}$.
The two expressions differ only in notation (\emph{truth} vs.
\emph{ground\_truth}), not in meaning.
...
\[
  \text{WER} =
  \frac{\text{substitutions}_{w} + \text{deletions}_{w}
    + \text{insertions}_{w}}{|\mathit{ground\_truth}_{w}|}
\]
Equivalently,
$\text{WER}=\text{edit\_distance}_{w}(\mathit{pred},\mathit{truth})/|\mathit{truth}_{w}|$,
where the edit distance is computed over word tokens (not characters).
WER is typically higher than CER because a single word-level error can
aggregate multiple character-level deviations.
...
\]
where predictions are partitioned into $M$ confidence bins $B_m$,
$n$ is the total number of samples, $\operatorname{acc}(B_m)$ is
empirical accuracy in bin $m$, and $\operatorname{conf}(B_m)$ is mean
predicted confidence in bin $m$. Lower ECE indicates better
...
discrimination. This was initially intentional to make the classifier able to handle all types of inputs.
...
Google Vision serve as cloud comparison points.

\paragraph{Hardware.}
All benchmarks were executed in the cloud on a Google Cloud \texttt{n4-highcpu-8} instance.
Latency metrics are infrastructure-dependent; reproducibility across
different instance types requires re-measurement.
...
\begin{itemize}
  \item \textbf{Dataset bias}: Training on NIST and IAM may not
    ...
    characteristics, although custom sets representing combinations of the data were used.
...
intervals, unresolved TrOCR anomaly, and architecture-level ablation
gaps.
